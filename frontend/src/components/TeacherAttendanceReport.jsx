...
const TeacherAttendanceReport = () => {
    ...
    const handleSearch = async () => {
        ...
        try {
            ...
            if (view === 'summary') {
                ...
                const res = await api.get(`/attendance/matrix?${params.toString()}`);
                setMatrix(res.data);
            }
        } catch (error) {
    ...
    const exportToExcel = () => {
        ...
        if (view === 'summary' && stats?.daily?.length) {
            ...
            XLSX.utils.book_append_sheet(workbook, ws, 'វត្តមានគ្រូ');
            XLSX.writeFile(workbook, `Teacher_Attendance_${fromDate}_to_${toDate}.xlsx`);
        } else if (view === 'matrix' && matrix?.dates?.length && matrix?.people?.length) {
            const headers = ['ឈ្មោះ', ...matrix.dates.map(d => formatDateDisplay(d))];
            ...
            XLSX.utils.book_append_sheet(workbook, ws, 'វត្តមានគ្រូ');
            XLSX.writeFile(workbook, `Teacher_Attendance_Matrix_${fromDate}_to_${toDate}.xlsx`);
        }
    };
    ...
    const renderSummary = () => {
        ...
        return (
            <div className="bg-white p-6 rounded-lg shadow-md">
                <h2 className="text-xl font-bold mb-4">វត្តមានគ្រូ</h2>
    ...
    return (
        <div className="p-4 space-y-6">
            <div className="flex justify-between items-center">
                <h1 className="text-2xl font-bold">របាយការណ៍វត្តមានគ្រូ</h1>
                {canExport && (
