...
const StudentAttendance = () => {
    ...
    const filteredAttendance = useMemo(() => {
        ...
        return attendanceData.filter((entry) => entry.student?.class?._id === selectedClass);
    ...
    const handleStatusChange = (studentId, newStatus) => {
        setAttendanceData((prev) =>
            prev.map((entry) => {
                if (entry.student._id === studentId) {
                    const updated = {
                        ...entry,
    ...
    const handleCheckInChange = (studentId, value) => {
        setAttendanceData((prev) =>
            prev.map((entry) => {
                if (entry.student._id === studentId) {
                    const updated = {
    ...
    const handleCheckOutChange = (studentId, value) => {
        setAttendanceData((prev) =>
            prev.map((entry) => {
                if (entry.student._id === studentId) {
                    const updated = {
    ...
    const handleMarkAll = (status) => {
        showConfirmDialog({
            ...
            onConfirm: () => {
                const visibleIds = new Set(filteredAttendance.map((entry) => entry.student._id));
                setAttendanceData((prev) =>
                    prev.map((entry) =>
                        visibleIds.has(entry.student._id)
                            ? {
                                ...entry,
                                attendance: entry.attendance
                                    ? { ...entry.attendance, status }
                                    : {
                                        _id: `temp-${entry.student._id}`,
    ...
    const handleSave = async () => {
        ...
        try {
            setIsSaving(true);
            await api.post('/student-attendance', { records });
    ...
    const fetchHistory = async () => {
        ...
        try {
            ...
            if (selectedSchool) params.schoolId = selectedSchool;
            const { data } = await api.get('/student-attendance/history', { params });
            setHistoryData(data);
        } catch (error) {
    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <h1 className="text-2xl font-bold text-gray-800">វត្តមានសិស្ស</h1>
                ...
                <div className="flex flex-col md:flex-row gap-4 items-end">
                    ...
                    <div>
                        <select
                            ...
                            className="w-full p-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                            disabled={!selectedSchool && user.role !== 'school-admin'}
                        >
            ...
            <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
                <StatCard label="សិស្សសរុប" value={summary.total} icon={<FaCheckCircle className="text-gray-500" />} colorClass="border-gray-500" />
            ...
            <div className="bg-white rounded-lg shadow-sm overflow-hidden">
                <div className="p-4 border-b border-gray-200 flex flex-wrap items-center justify-between gap-3">
                    <h2 className="font-semibold text-gray-700">បញ្ជីសិស្ស</h2>
                ...
                {!hasSearched ? (
                    ...
                    <div className="text-center py-16 text-gray-500">
                        <FaCalendarDay className="mx-auto text-4xl mb-3 text-gray-300" />
                        <p>គ្មានទិន្នន័យសិស្សសម្រាប់ថ្នាក់នេះទេ។</p>
                    </div>
                ) : (
                        ...
                        <table className="min-w-full bg-white">
                            <thead>
                                <tr className="bg-gray-50 text-left text-sm font-medium text-gray-600">
                                    ...
                                    <th className="py-3 px-4 border-b">លេខសម្គាល់សិស្ស</th>
                                    <th className="py-3 px-4 border-b">គោត្តនាម-នាម (ខ្មែរ)</th>
                                    <th className="py-3 px-4 border-b">ភេទ</th>
                                    <th className="py-3 px-4 border-b">ថ្នាក់</th>
                            ...
                            <tbody>
                                {filteredAttendance.map((entry, index) => {
                                    ...
                                    return (
                                        <tr key={s._id} className="border-b hover:bg-gray-50 text-sm">
                                            <td className="py-3 px-4 text-gray-500">{index + 1}</td>
                                            <td className="py-3 px-4 font-medium">{s.studentId}</td>
                                            <td className="py-3 px-4">{s.fullNameKh}</td>
                                            <td className="py-3 px-4">{s.gender}</td>
                                            <td className="py-3 px-4">{s.class?.className || 'N/A'}</td>
                                            ...
                                            <td className="py-3 px-4">
                                                <input
                                                    type="time"
                                                    value={a?.checkIn || ''}
                                                    onChange={(e) => handleCheckInChange(s._id, e.target.value)}
                                            ...
                                            <td className="py-3 px-4">
                                                <input
                                                    type="time"
                                                    value={a?.checkOut || ''}
                                                    onChange={(e) => handleCheckOutChange(s._id, e.target.value)}
                                                    className="w-24 px-2 py-1 border border-gray-300 rounded text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                                                />
                                            </td>
                                            <td className="py-3 px-4">
                                                <input
                                                    type="text"
                                                    value={a?.note || ''}
                                                    onChange={(e) => handleNoteChange(s._id, e.target.value)}
                                                    placeholder="..."
                ...
                <div className="p-4 border-t border-gray-200 flex justify-end">
                    <button
                        onClick={handleSave}
                        disabled={isSaving || isLoading || filteredAttendance.length === 0}
                            ...
                            <table className="min-w-full bg-white border">
                                <thead>
                                    <tr className="bg-gray-50 text-left text-sm font-medium text-gray-600">
                                        ...
                                        <th className="py-2 px-3 border-b">សិស្ស</th>
                                ...
                                <tbody>
                                    {historyData.map((rec) => (
                                        <tr key={rec._id} className="border-b hover:bg-gray-50 text-sm">
                                            ...
                                            <td className="py-2 px-3">{rec.student?.fullNameKh || 'N/A'}</td>
